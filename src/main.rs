...
fn main() {
    ...
    let exec_strategy = match matches
        .get_one::<String>("parallel-strategy")
        .map(|s| s.as_str())
    {
    ...
    let chunk_size = if let Some(cs) = matches.get_one::<String>("chunk-size").map(|s| s.as_str()) {
    ...
    let num_workers = match matches.get_one::<String>("jobs").map(|s| s.as_str()) {
    ...
    let (escaper, output_sep, output_record_sep) = match matches
    ...
    let out_file = matches.get_one::<String>("out-file").map(|s| s.as_str());
    macro_rules! with_io {
